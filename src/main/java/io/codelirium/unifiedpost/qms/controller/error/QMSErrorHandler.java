package io.codelirium.unifiedpost.qms.controller.error;
...
import static io.codelirium.unifiedpost.qms.controller.mapping.UrlMappings.API_ERROR;
import static io.codelirium.unifiedpost.qms.domain.dto.response.builder.RESTResponseBodyBuilder.failure;
...
@ControllerAdvice
public class QMSErrorHandler extends BasicErrorController {

	private static final Logger LOGGER = getLogger(QMSErrorHandler.class);
	...
	@Inject
	public QMSErrorHandler(final ObjectMapper objectMapper) {
	...
	public @ResponseBody ResponseEntity<RESTFailureResponseBody<String>> handleException(final Exception e) {
		...
		try {
			...
			} else if (e instanceof DataIntegrityViolationException || e instanceof EmptyResultDataAccessException) {

				body   = failure(e.getClass().getSimpleName(), "There is a problem with the data store operation.");
